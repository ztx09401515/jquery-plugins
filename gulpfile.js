...
const tsxoptions={presets: ['@babel/preset-env'],
...
var tran=function (ext,babelOptions,src,lib) {
    return function() {
        gulp.src(src+'/**/*'+ext, {
            buffer: true
        }).pipe(through.obj(function (chunk, enc, next) {
            var buf = transform(chunk.contents, babelOptions)
            chunk.contents = buf;
            this.push(chunk)
            next();
        }))
            .pipe(gulp.dest(function (file) {
                ...
                var libPath=file.cwd+'\\'+lib;
                var relative = file.history[1].replace(cwdstring, '');
                relative = relative.replace(/\\[^\\]+$/, '');
                return lib?lib:file.base;

            }))
    }
}
gulp.task('babel-ts-lib', tran('.ts',tsxoptions,'src','lib'));
gulp.task('babel-ts', tran('.ts',tsxoptions,'src',''));
